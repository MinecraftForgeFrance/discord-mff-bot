    ...
    run(client, messageUser, message, args) {
        if (message.channel.name === defaultConfig.channels.supportModding || message.channel.name === defaultConfig.channels.supportProgramming || message.channel.name === defaultConfig.channels.bots) {
            if (args.length === 0)
                ...
                if (args[0] === 'on') {
                    message.guild.members.get(messageUser).addRole(role).then(() => logger.info(`${message.author.username} a le role ${defaultConfig.roles.roleSupport}`)).catch(console.error);
                }
                else if (args[0] === 'off') {
                    message.guild.members.get(messageUser).removeRole(role).then(() => logger.info(`${message.author.username} n'a plus le role ${defaultConfig.roles.roleSupport}`)).catch(console.error);
                }
                else {
                    message.channel.send("La syntaxe doit être `!support \<\on/off\>`.").then(async (message) => logger.info(`Send message : ${message.content}`)).catch(console.error);
                }
            }
        }
        else {
