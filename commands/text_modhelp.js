...
module.exports = {
    run(client, messageUser, message, args) {
        if (args.length === 0)
            ...
            request({
                uri: `${defaultConfig["protocol"]}://${defaultConfig["hostname"]}:${defaultConfig["port"]}/discordapi/solvedthread?term=${supportName}${(args[0].startsWith("-v")) ? `&hasTags[]=${supportVersion}` : ''}&token=${defaultConfig["token"]}`,
                json: true
            }, (err, res, body) => {
