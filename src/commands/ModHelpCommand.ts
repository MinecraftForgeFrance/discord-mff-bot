...
export class ModHelpCommand extends Command {
    ...
    public perform(sender: UserInfo, ctx: CommandContext, resolve: () => void, reject: () => void): void {
        ...
        request({
            uri: `${config.get("forumLink.protocol")}://${config.get("forumLink.hostname")}:${config.get("forumLink.port")}/discordapi/solvedthread?term=${search}${tagsParameter}&token=${config.get("forumLink.token")}`,
            json: true,
        }, (err, res, body) => {
